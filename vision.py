...
#             previous_cycle_time = time.perf_counter()
            ...
            if(self._mode.value == Mode.HAND_COPY.value and not need_to_calibrate and init_motors):
                self.t_accum.value = (1 - (thumb_n[0] + thumb_n[1] + thumb_n[2])/3)    * 4096 * 1.7
                self.i_accum.value = (1 - (index_n[0] + index_n[1] + index_n[2])/3)    * 4096 * 2
                self.m_accum.value = (1 - (middle_n[0] + middle_n[1] + middle_n[2])/3) * 4096 * 2.1
                self.r_accum.value = (1 - (ring_n[0] + ring_n[1] + ring_n[2])/3)       * 4096 * 1.9
                self.p_accum.value = (1 - (pinkie_n[0] + pinkie_n[1] + pinkie_n[2])/3) * 4096 * 1.7             
